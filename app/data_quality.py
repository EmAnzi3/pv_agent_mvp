...
def repair_obvious_fields(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    - corregge MW solo quando il campo è chiaramente sottoscala rispetto al titolo;
    - non corregge province discordanti, perché spesso i progetti attraversano più province
      o il titolo cita anche opere di connessione.
    """
    ...
    for idx, row in enumerate(rows):
        ...
        if should_repair_mw_from_title(c["mw"], title_mw):
            ...
            repairs.append({
                ...
                "reason": "field_mw_underscaled_vs_title",
                "title": c["title"],
                "source": c["source"],
                "province": c["province"],
                "url": c["url"],
        ...
        if title_prov and not c2["province_norm"]:
            ...
            set_field_if_present(new_row, PROVINCE_FIELDS, title_prov)
            repairs.append({
                "idx": idx,
                "field": "province",
                "old_value": old_value,
                "old_mw": c2["mw"],
                "new_value": title_prov,
                "reason": "province_filled_from_explicit_title_code",
                "title": c2["title"],
                "source": c2["source"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit, correzioni conservative e deduplica sicura V8 per pv_agent_mvp data.json")
    ...
    print(f"[data-quality-v8] input: {input_path}")
    print(f"[data-quality-v8] project list key: {project_key or '<root-list>'}")
    print(f"[data-quality-v8] record iniziali: {len(rows)}")
    ...
    print(f"[data-quality-v8] record dopo deduplica: {len(deduped_rows)}")
    print(f"[data-quality-v8] fusi/rimossi: {len(rows) - len(deduped_rows)}")
    print(f"[data-quality-v8] correzioni automatiche: {len(repairs)}")
    print(f"[data-quality-v8] coppie accettate: {len(accepted)}")
    print(f"[data-quality-v8] coppie respinte: {len(rejected)}")
    print(f"[data-quality-v8] righe sospette: {len(suspicious)}")
    print(f"[data-quality-v8] esclusi dalla top_projects perché sospetti: {len(excluded_keys)}")
    print(f"[data-quality-v8] project_key splittate per conflitto: {len(project_key_splits)}")
    print(f"[data-quality-v8] output: {output_path}")
    print(f"[data-quality-v8] audit: {reports_dir / 'dedupe_audit.md'}")

    if args.in_place:
        backup = input_path.with_name(input_path.stem + f"_backup_before_dedupe_v8_{now_stamp()}" + input_path.suffix)
        shutil.copy2(input_path, backup)
        shutil.copy2(output_path, input_path)
        print(f"[data-quality-v8] backup creato: {backup}")
        print(f"[data-quality-v8] data.json sovrascritto: {input_path}")
    return 0
