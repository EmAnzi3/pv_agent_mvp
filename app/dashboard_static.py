      ...
      <div class="card">
        ...
        >
          <div class="small muted">
            ...
            ? limite visualizzazione: 700 righe
          </div>
  ...
  <script>
    ...
    function exportFilteredCsv() {
      ...
      const rows = filtered.map(record => [
        record.source_label
          || sourceLabel(record.source_group || record.source),
        record.region,
        record.province,
        record.municipalities,
        record.title,
        ...
        record.power_mw === null
          ...
          || record.power_mw === ''
            ? ''
            : formatItalianNumber(record.power_mw, 3),
        record.status,
        safeUrl(record.url)
      ]);
    ...
    function renderPipeline() {
      ...
      if (exportButton) {
        ...
        exportButton.textContent = filtered.length
          ? `Esporta CSV (${fmtNum(filtered.length)})`
          : 'Nessun risultato';
      }
    ...
    const pipelineExportButton = document.getElementById(
      'exportFilteredCsv'
    );
