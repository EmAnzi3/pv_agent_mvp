...
def manual_exclusion_match(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for rule in MANUAL_EXCLUDE_COMPLETED_SMALL_PROJECTS:
        if c["source_norm"] != rule["source_norm"]:
            continue
        if c["title_norm"] != rule["title_norm"]:
            continue
        municipality_rule = rule.get("municipality_norm")
        if municipality_rule and c["municipality_norm"] and c["municipality_norm"] != municipality_rule:
            continue
        return rule
    return None
        ...
        if new_proponent:
            ...
            if norm_text(old_proponent) != norm_text(new_proponent):
                ...
                actions.append({
                    ...
                    "reason": "manual_proponent_override_by_url",
                    "action": "updated",
                })
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit, correzioni conservative, override manuali e deduplica sicura V13 per pv_agent_mvp data.json")
    ...
    print(f"[data-quality-v13] input: {input_path}")
    print(f"[data-quality-v13] project list key: {project_key or '<root-list>'}")
    print(f"[data-quality-v13] record iniziali: {len(rows)}")
    ...
    print(f"[data-quality-v13] record dopo deduplica: {len(deduped_rows)}")
    print(f"[data-quality-v13] fusi/rimossi: {len(rows) - len(deduped_rows)}")
    print(f"[data-quality-v13] correzioni automatiche/manuali: {len(repairs)}")
    print(f"[data-quality-v13] azioni manuali: {len(manual_actions)}")
    print(f"[data-quality-v13] coppie accettate: {len(accepted)}")
    print(f"[data-quality-v13] coppie respinte: {len(rejected)}")
    print(f"[data-quality-v13] righe sospette: {len(suspicious)}")
    print(f"[data-quality-v13] esclusi dalla top_projects perché sospetti: {len(excluded_keys)}")
    print(f"[data-quality-v13] project_key splittate per conflitto: {len(project_key_splits)}")
    print(f"[data-quality-v13] output: {output_path}")
    print(f"[data-quality-v13] audit: {reports_dir / 'dedupe_audit.md'}")
    ...
    if args.in_place:
        ...
        print(f"[data-quality-v13] backup creato: {backup}")
        print(f"[data-quality-v13] data.json sovrascritto: {input_path}")
    return 0
