...
class SiciliaCollector(BaseCollector):
    ...
    def _scan_known_municipalities(self, text: str) -> list[str]:
        ...
        # "Francofonte (SR), provincia di Siracusa" -> non aggiungere Siracusa
        # "Mezzojuso e Ciminna, provincia di Palermo" -> non aggiungere Palermo
        for province in province_names:
            scan_norm = re.sub(rf"\bprovincia\s+(?:di|del|della)?\s*{re.escape(province)}\b", " ", scan_norm)
            scan_norm = re.sub(rf"\bprov\.\s*(?:di|del|della)?\s*{re.escape(province)}\b", " ", scan_norm)
        ...
        for municipality_norm in sorted(SICILIA_MUNICIPALITY_TO_PROVINCE, key=len, reverse=True):
            if re.search(rf"\b{re.escape(municipality_norm)}\b", scan_norm):
