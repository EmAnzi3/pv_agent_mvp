...
class SiciliaCollector(BaseCollector):
    ...
    def _fetch_detail_info(self, url: str, debug_base: Path) -> dict:
        ...
        try:
            ...
            title = self._extract_detail_title(soup, plain)
            status_raw = self._extract_status_from_lines(lines, plain)
            proponent = self._extract_proponent_from_text(plain)
            province = self._extract_province(plain)
            municipalities = self._extract_municipalities(plain)
            power = self._extract_power_text(plain)

            return {
                "title": title,
                "plain_text_sample": plain[:5000],
                "status_raw": status_raw,
                "proponent": proponent,
                "province": province,
                "municipalities": municipalities,
    ...
    def _merge_detail_info(self, normalized: dict, detail_info: dict) -> dict:
        ...
        # Usa il titolo dettaglio solo se è davvero più informativo e parla dello stesso ambito.
        if (
            detail_title
            and len(detail_title) > len(current_title)
    ...
    def _extract_municipalities(self, text: str) -> list[str]:
        ...
        # Pattern più affidabile: "Comune (XX)" ovunque nel dettaglio.
        for match in re.finditer(
            r"\b([A-ZÀ-Ý][A-Za-zÀ-ÿ'’\-\s]{2,55}?)\s*\((AG|CL|CT|EN|ME|PA|RG|SR|TP)\)",
            value,
            flags=re.IGNORECASE,
        ):
            comune = self._clean_municipality(match.group(1))
            if comune and comune not in found:
                found.append(comune)

        patterns = [
            r"\bnei\s+comuni\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bnel\s+comune\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bcomuni\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bcomune\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bterritori\s+comunali\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bterritorio\s+del\s+comune\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bsito\s+nel\s+comune\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bda\s+realizzarsi\s+nel\s+comune\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
            r"\bda\s+realizzarsi\s+nei\s+comuni\s+di\s+(.+?)(?:\s+(?:e\s+relative|e\s+delle\s+relative|con\s+opere|opere\s+di|provincia|c\.da|contrada|localit[aà]|foglio|particella|particelle|potenza|denominato|denominata)|[.;]|$)",
        ]
        ...
        for pattern in patterns:
            for match in re.findall(pattern, value, flags=re.IGNORECASE):
                ...
                # Se nel chunk ci sono "Comune (XX)", catturali prima.
                for code_match in re.finditer(
                    r"\b([A-ZÀ-Ý][A-Za-zÀ-ÿ'’\-\s]{2,55}?)\s*\((AG|CL|CT|EN|ME|PA|RG|SR|TP)\)",
                    chunk,
                    flags=re.IGNORECASE,
                ):
                    comune = self._clean_municipality(code_match.group(1))
                    if comune and comune not in found:
                        found.append(comune)

                chunk = re.sub(r"\((AG|CL|CT|EN|ME|PA|RG|SR|TP)\)", " ", chunk, flags=re.IGNORECASE)
                chunk = re.split(
                    r"\b(?:c\.da|contrada|localit[aà]|provincia|distinto|foglio|particella|particelle|snc|con potenza|elettrodotto|cavidotto|rtn|stazione|cabina)\b",
                    chunk,
                    flags=re.IGNORECASE,
                )[0]

                parts = re.split(r",|;|\se\s|\sed\s|\s-\s|/", chunk)
    ...
    def _clean_municipality(self, value: str) -> str | None:
        ...
        value = re.sub(r"^(?:comune\s+di|comuni\s+di|di|del|della|dello|dei|degli|in|nel|nella)\s+", "", value, flags=re.IGNORECASE)
        ...
        if len(value) < 3 or len(value) > 65:
            return None
        ...
        if normalized in {"sicilia", "provincia", "comune", "comuni"}:
            return None
