...
<body>
  ...
  <main>
    <section>
      ...
      <div class="grid kpi-grid">
        ...
        <div class="card kpi">
          ...
          <div class="note" id="kpiQualityNote">Controlli data quality</div>
        </div>
      </div>
    </section>
    ...
    <section>
      <h2 class="section-title">Pipeline operativa vs radar Terna</h2>
    ...
    <section>
      ...
      <div class="grid quality-grid">
        <div class="card kpi">
          <div class="label">Righe sospette</div>
          <div class="value" id="qualitySuspicious">-</div>
          <div class="note">Devono restare a zero</div>
        </div>
        <div class="card kpi">
          <div class="label">Project key duplicate</div>
          <div class="value" id="qualityDuplicateKeys">-</div>
          <div class="note">Calcolate lato dashboard</div>
        </div>
        <div class="card kpi">
          <div class="label">Fonte Puglia CKAN</div>
          <div class="value" id="qualityPugliaCkan">-</div>
          <div class="note">Deve restare esclusa</div>
        </div>
        <div class="card kpi">
          <div class="label">Data quality</div>
          <div class="value" id="qualityVersion">-</div>
          <div class="note">Versione script applicata</div>
        </div>
        <div class="card kpi">
          <div class="label">Senza proponente</div>
          <div class="value" id="qualityMissingProponent">-</div>
          <div class="note">Solo pipeline operativa</div>
        </div>
      </div>

      <div class="split" style="margin-top: 18px;">
        <div class="card">
          <h3 style="margin-top: 0;">Qualità per fonte</h3>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th class="num">Senza MW</th>
                  <th class="num">Senza provincia</th>
                  <th class="num">Senza comune</th>
                  <th class="num">Completezza</th>
                </tr>
              </thead>
        ...
        <div class="card">
          <div class="quality-box">
            <strong>Stato dati:</strong>
            <ul class="mini-list">
              <li>Pipeline operativa separata dal radar Terna.</li>
              <li>Fonte Puglia CKAN esclusa dalla pipeline.</li>
              <li>Deduplica MASE / MASE Provvedimenti gestita in data quality.</li>
              <li>Top Projects esclusivamente da record puntuali.</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    const mwFmt = new Intl.NumberFormat('it-IT', { maximumFractionDigits: 1 });
    ...
    function initKpis() {
      ...
      const suspicious = dataQuality.suspicious_rows ?? 0;
      const duplicateKeys = countDuplicateProjectKeys();
      const pugliaCkan = countSource('puglia');
      ...
      const qualityOk = suspicious === 0 && duplicateKeys === 0 && pugliaCkan === 0;
      setText('kpiQuality', qualityOk ? 'OK' : 'Check');
      setText('kpiQualityNote', `sospette: ${suspicious} · dup key: ${duplicateKeys}`);

      setText('qualitySuspicious', fmtNum(suspicious));
      setText('qualityDuplicateKeys', fmtNum(duplicateKeys));
      setText('qualityPugliaCkan', pugliaCkan === 0 ? '0' : fmtNum(pugliaCkan));
      setText('qualityVersion', dataQuality.version || 'n/d');
      setText('qualityMissingProponent', fmtNum(missingProponent));
    }
    ...
    function initCharts() {
      ...
      new Chart(document.getElementById('punctualMwChart'), {
        ...
        data: {
          ...
          datasets: [{ label: 'MW puntuali', data: topPunctualMw.map(x => x.punctual_mw) }]
        },
        options: chartOptions('Pipeline operativa: MW puntuali per regione')
      });
      ...
      new Chart(document.getElementById('ternaMwChart'), {
        ...
        options: chartOptions('Radar Terna: MW solare per regione')
      });
    ...
    function attachEvents() {
      ['searchInput', 'dataKindFilter', 'sourceFilter', 'regionFilter', 'provinceFilter', 'statusFilter'].forEach(id => {
